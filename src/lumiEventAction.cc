...
void lumiEventAction::BeginOfEventAction(const G4Event*){
  ...
  // CollID = sdManager->GetCollectionID("SensDet/LumiHitColl");
...
void lumiEventAction::EndOfEventAction(const G4Event* anEvent){
  ...
  for( decltype(n) hcidx =0; hcidx<n;hcidx++){
    lumiDetectorHitsCollection* hitsCollection = (lumiDetectorHitsCollection*)(hce->GetHC(hcidx));
    ...
    for(int ihit=0;ihit<nhits;ihit++){
      ...
      creatorID = 0; //FIXME
      const G4String creatorProcess =(*hitsCollection)[ihit]->GetCreatorProcess();
      ...
      analysisManager->FillNtupleSColumn(15,creatorProcess);
  ...
  } // end of loop over hit collections of this events
